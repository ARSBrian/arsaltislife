...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([west,getPos player,30] call life_fnc_nearUnits) && (player getVariable["restrained",FALSE]) && vehicle player == player) exitWith {
			...
			detach player;
			titleText["You have been set free, because the time has expired.","PLAIN"];
...
titleText[format["You have been arrested by %1.",_cop getVariable["realname",name _cop]],"PLAIN"];
